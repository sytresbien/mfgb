...
from dataset import Inference_Dataset, Inference_M2V_Dataset
import os
import rdkit
...
if __name__ == '__main__':
    df = pd.read_csv('data/clf/bace.csv')
    ds = Inference_M2V_Dataset(df['SMILES'].tolist()).get_data()

    medium = {'name': 'Medium', 'num_layers': 6, 'num_heads': 8, 'd_model': 256, 'path': 'weights_mol2vec_hs_balanced', 'addH': True}
    arch = medium
    trained_epoch = 3
    ...
    model.load_weights(arch['path']+'/bert_weights_{}_{}.h5'.format(arch['name'],trained_epoch))
